...
export async function buildRunRateExcelBuffer(matrix: RunRateMatrix): Promise<Buffer> {
  ...
  const totalCols = 2 + numWeeks + 8 // SKU + Catálogo + W1..Wn + Total + Prom/sem + Hist/sem + Δ Hist + ABC + XYZ + Lifecycle + Driver
  ...
  monthHeaderRow.push(
    cell('', S.MONTH_HEADER), cell('', S.MONTH_HEADER), cell('', S.MONTH_HEADER), cell('', S.MONTH_HEADER), cell('', S.MONTH_HEADER),
    cell('', S.MONTH_HEADER), cell('', S.MONTH_HEADER), cell('', S.MONTH_HEADER),
  )
  ...
  headerRow.push(
    ...
    cell('ABC', S.COL_HEADER),
    cell('XYZ', S.COL_HEADER),
    cell('Lifecycle', S.COL_HEADER),
  ...
  matrix.rows.forEach((r, i) => {
    ...
    const centered = alt
      ? { ...S.BODY_ALT, alignment: { horizontal: 'center', vertical: 'center' } }
      : { ...S.BODY, alignment: { horizontal: 'center', vertical: 'center' } }
    row.push(
      ...
      cell(r.abc_class, centered),
      cell(r.xyz_class, centered),
      cell(r.lifecycle_stage, leftStyle),
