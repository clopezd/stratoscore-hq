...
function Card({ label, value, sub, icon, accent, trend }: CardProps) {
  const trendColor = trend === 'up' ? 'text-emerald-400' : trend === 'down' ? 'text-red-400' : 'text-vid-muted'
  return (
    <div className="relative overflow-hidden bg-vid-card border border-vid rounded-xl p-4">
      <div className="absolute inset-x-0 top-0 h-[1px] bg-gradient-to-r from-transparent via-white/10 to-transparent" />
      <div className={`inline-flex p-2 rounded-lg mb-3 ${accent}`}>
        {icon}
      </div>
      <p className="text-[10px] text-vid-subtle uppercase tracking-wider mb-0.5">{label}</p>
      <p className="text-2xl font-bold text-vid-fg">{value}</p>
