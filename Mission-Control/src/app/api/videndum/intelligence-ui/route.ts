...
export async function POST() {
  ...
  if (authError) {
    console.error('[intelligence-ui] Supabase auth error:', authError.message)
    return NextResponse.json({ error: `Auth error: ${authError.message}` }, { status: 401 })
  }
  if (!user) {
    console.error('[intelligence-ui] No authenticated user found')
    return NextResponse.json({ error: 'Unauthorized: no active session' }, { status: 401 })
  }

  // ── Verify OpenRouter key ───────────────────────────────────────────────────
  if (!process.env.OPENROUTER_API_KEY) {
    console.error('[intelligence-ui] OPENROUTER_API_KEY no configurado')
    return NextResponse.json({ error: 'OPENROUTER_API_KEY no configurado en el servidor' }, { status: 500 })
  }

  try {
    // ── 1. Revenue trend 2022 vs 2024 ─────────────────────────────────────────
    console.log('[intelligence-ui] Querying revenue trend for user:', user.email)
    ...
    console.log('[intelligence-ui] DB queries OK — launching LLM analysis')
...
1. DJI (China): Ronin 4D, RS4 Pro → vs. Manfrotto gimbals, Sachtler heads | IMPACTO ALTO
2. SmallRig (China): 180+ SKUs, 30-70% más barato → vs. Wooden Camera, Manfrotto | IMPACTO ALTO
3. Hollyland (China): transmisores inalámbricos 20-30% precio Teradek | IMPACTO ALTO
4. Tilta (China): calidad creciente → vs. cages, power systems | IMPACTO MEDIO-ALTO
5. Aputure (China): LED profesional → vs. Litepanels, Quasar Science | IMPACTO ALTO
6. ARRI (Alemania): ultra-premium → vs. Sachtler/O'Connor premium | IMPACTO MEDIO
...

{
  ...
  "action_table": [
    ...
    console.log('[intelligence-ui] LLM response received, parsing JSON')
